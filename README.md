...
| **Services**   | Tavily search, Cohere rerank, OpenAI moderation (the `services` config toggles: `search`, `rerank`, `moderate`)                                                                                                                                                                                       | [Services](https://aimock.copilotkit.dev/services)  |
...
# (vidaimock input: a template file or directory; mockllm input: one YAML file.
#  The optional output is a single JSON file — omit it to print to stdout)
...
npx @copilotkit/aimock validate --strict --json ./fixtures/openai.json
