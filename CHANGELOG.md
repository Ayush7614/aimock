...
- **`GET` / `POST` / `DELETE /__aimock/chaos` — read, set and clear chaos at runtime**, no restart. `POST` accepts any subset of `dropRate` / `malformedRate` / `disconnectRate` in `[0, 1]`; unknown fields and out-of-range values are `400`. **Overrides are scoped to the caller's `X-Test-Id`**, the way fixture match-counts and video job state already are: chaos installed by test `t1` applies only to traffic carrying `X-Test-Id: t1`, so one test cannot fail the tests running beside it on the shared `aimock` process that a parallel suite points at. An untagged call sets the server-wide baseline. Both sides — the control call and the traffic it affects — resolve the tag the same way the rest of the server does (the `X-Test-Id` header, else `?testId=` in the query string), so the two can never resolve to different scopes. `DELETE` drops one override; `POST /__aimock/reset` drops them all, restoring the chaos config the server was started with, so a leaked setting can never survive the isolation barrier and a `--chaos-drop` baseline is always recoverable (#435)

### Fixed

- **Moderation, rerank and web search honour strict mode (#440).** `POST /v1/moderations`, `POST /v2/rerank` and `POST /search` were the three handlers that ignored `--strict` / `X-AIMock-Strict` entirely: an unmatched request still answered the lenient default — `results: []`, or an unflagged moderation result — with a `200`, so a strict CI suite that hit one of these endpoints with no registered fixture went green on a response the mock invented. They now return the same `503` `no_fixture_match` envelope every other fixture-matching handler returns, before any default is built. **A strict suite that relied on the lenient default will start failing** — that is the point; register a fixture (or a `/.*/` catch-all) for the request, or send `X-AIMock-Strict: false`. Config-file users are unaffected: enabling `search` / `rerank` / `moderate` in `aimock.json` registers a `/.*/` catch-all, so those services never miss. The journal records `strictOverride` on both limbs — the `503` and the header opt-out's `200` — like every other strict-aware surface.
- **Every wrapper response path now leaves a journal entry, and A2A entries are filterable (#441).** Paths that previously answered a request and recorded nothing are now journaled: the vector malformed-JSON `400` (mounted and standalone) and the vector unmatched-path `404` (standalone, and mounted — where the parent server writes the 404 — so both modes agree), the A2A agent-card fetch, A2A JSON-RPC parse errors and unmatched streaming messages, and the A2A / MCP / AG-UI standalone `500` catch blocks plus the AG-UI standalone unhandled-request `404`. **A consumer asserting an exact journal length or reading `X-Total-Count` for traffic on those paths will see larger numbers than before**; `VectorMock`'s standalone `404` in particular flipped from journaling nothing to journaling one entry. A2A entries also record `req.url` rather than the mount-relative, query-stripped sub-path, the way vector, MCP and AG-UI already do, so `GET /__aimock/journal?path=/a2a` and `?testId=` (which falls back to parsing `?testId=` out of the recorded path) now select A2A traffic instead of silently returning nothing. Each request still produces exactly one entry. Journals created by hand for a standalone mock are unbounded by default, so a long-running mock taking unmatched scanner traffic now grows where it did not before — cap it with `journalMaxEntries` (server mode already defaults to 1000).
- **A JSON body that parses but is not an object is answered with `400`, on every provider JSON handler and on `VectorMock` (#438).** `JSON.parse("null")` succeeds, so the malformed-JSON branch never ran and the first field read (`body.messages`, `body.prompt`, `body.input`, …) threw a `TypeError` that surfaced as a bare `500` — a mock server failing the way a broken server does, which reads as an aimock bug rather than a malformed request. Every provider JSON handler now answers `400 "Request body must be a JSON object"` for `null`, an array, or a scalar body, in each provider's own error envelope: OpenAI chat/responses/embeddings/moderations/images/speech/video, Anthropic Messages, Cohere chat/embed/rerank, Gemini `generateContent`/`streamGenerateContent`/`embedContent`/interactions, Ollama `/api/chat` / `/api/generate` / `/api/embeddings`, Bedrock invoke/converse (and their streaming forms), Azure deployments, ElevenLabs, fal and Tavily search. `VectorMock` is covered too, in both mounted and standalone mode, across all eight of its body-carrying routes (Pinecone `query` / `vectors/upsert` / `vectors/delete`, Qdrant `points/search` / `points` / `points/delete`, ChromaDB `query` / `add`) — there a `null` body did not merely answer `500`, it threw an uncaught `TypeError` that took the process down. The A2A, MCP and AG-UI JSON-RPC wrappers keep their own protocol-level error handling and are not part of this change. **Consumer-visible beyond the `500` → `400` fix:** `POST /v1/moderations`, `POST /search` and `POST /v2/rerank` used to answer `200` with a degenerate empty result for an array or scalar body (the missing field resolved to `""` and matched the empty fixture), `/v1beta/models/{model}:embedContent` likewise returned a `200` embedding, and seven routes answered a `404` rather than an error — both fal audio routes, `POST /v1/messages`, `POST /v1beta/interactions`, Gemini `:generateContent` and `:streamGenerateContent` (`404 No fixture matched`) and the vector `POST /query` (`404 Collection not found`) — all are now `400`. Counts measured against `v1.41.0` by replaying `null`, `[]`, `123`, `"hi"` and `true` at 41 routes on a mock with no fixtures registered: 205 requests, every one of them a `400` now, where before there were 44 misleading `200`s, 32 `500`s, 28 `404`s and 8 uncaught crashes. No provider accepts a non-object body at any of these routes, so no legal SDK request is affected; an `{}` body keeps its existing behaviour everywhere. The `400` carries no `code`: the body parsed, so `invalid_json` — which the adjacent parse-failure branch still emits — would be inaccurate.
...
- **`POST /__aimock/error` accepts `null` for `body.message` / `body.type` / `body.code` (#434).** aimock's own `serializeErrorResponse` emits `code: null`, and so does a real OpenAI error, so the natural move — paste a captured error body back in — was answered with `400`. `null` now reads as absent — for `body` itself too, so `{"status": 429, "body": null}`, which is what a serializer emits for an unset optional, queues the default envelope instead of returning `400`. `status: null` likewise keeps the historic default of 500.
- **The multipart boundary parser implements the RFC 2046 case quoting exists for (#434).** `extractBoundary` matched `boundary=([^\s;]+)`, which stops at the first whitespace, so `boundary="a b c"` yielded the useless `"a` — the `--` delimiter never matched and every form field silently fell back to its default (`whisper-1` on transcriptions, `dall-e-2` on `/v1/images/edits` and `/v1/images/variations`, likewise on `/v1/videos`), which reads as a missing fixture rather than a parse failure. The quotes are now consumed BY the match, so a space-bearing quoted boundary parses (`bchars := bcharsnospace / " "`, RFC 2046 §5.1.1). Only DQUOTE quotes: an apostrophe is a legal RFC 2045 token character and a legal RFC 2046 `bchar`, so `boundary='abc'` is a bare token whose value includes the quotes.
