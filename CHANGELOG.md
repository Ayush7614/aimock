...
- **`POST /__aimock/error` accepts `null` for `body.message` / `body.type` / `body.code` (#434).** aimock's own `serializeErrorResponse` emits `code: null`, and so does a real OpenAI error, so the natural move — paste a captured error body back in — was answered with `400`. `null` now reads as absent. `status: null` likewise keeps the historic default of 500.
