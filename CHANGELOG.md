...
- **A JSON body that parses but is not an object is answered with `400`, on every handler (#438).** `JSON.parse("null")` succeeds, so the malformed-JSON branch never ran and the first field read (`body.messages`, `body.prompt`, `body.input`, …) threw a `TypeError` that surfaced as a bare `500` — a mock server failing the way a broken server does, which reads as an aimock bug rather than a malformed request. Every JSON handler now answers `400 "Request body must be a JSON object"` for `null`, an array, or a scalar body, in each provider's own error envelope: OpenAI chat/responses/embeddings/moderations/images/speech/video, Anthropic Messages, Cohere chat/embed/rerank, Gemini `generateContent`/`embedContent`/interactions, Ollama `/api/chat` / `/api/generate` / `/api/embeddings`, Bedrock invoke/converse (and their streaming forms), ElevenLabs, fal and Tavily search. **Consumer-visible beyond the `500` → `400` fix:** `POST /v1/moderations`, `POST /search` and `POST /v2/rerank` used to answer `200` with a degenerate empty result for an array or scalar body (the missing field resolved to `""` and matched the empty fixture), `/v1beta/models/{model}:embedContent` likewise returned a `200` embedding, and the two fal audio routes answered `404 No fixture matched` — all are now `400`. No provider accepts a non-object body at any of these routes, so no legal SDK request is affected; an `{}` body keeps its existing behaviour everywhere. The `400` carries no `code`: the body parsed, so `invalid_json` — which the adjacent parse-failure branch still emits — would be inaccurate.
